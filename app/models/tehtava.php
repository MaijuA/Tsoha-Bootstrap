...
//        self::disconnect_luokat($id);
        $query = DB::connection()->prepare('UPDATE Tehtava SET (nimi, kayttaja_id, prioriteetti, status, luokka_id, kuvaus) = (:nimi, :kayttaja_id, :prioriteetti, :status, :luokka_id, :kuvaus) WHERE id=:id');
        $query->execute(array('id' => $id, 'nimi' => $this->nimi, 'kayttaja_id' => $this->kayttaja_id, 'prioriteetti' => $this->prioriteetti, 'status' => $this->status, 'luokka_id' => $this->luokka_id, 'kuvaus' => $this->kuvaus));
        $destroyconnections = DB::connection()->prepare('DELETE FROM TehtavaLuokka WHERE tehtava_id = :tehtava_id');
        $destroyconnections->execute(array('tehtava_id' => $this->id));
        $row = $query->fetch();
    }
